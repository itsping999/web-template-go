...
type SQLiteDB struct{ Conn *gorm.DB }

func NewSQLiteDB(cfg SQLiteConfig, logger *logrus.Entry) (SQLiteDB, func(), error) {
	if !cfg.Enabled {
		return SQLiteDB{}, nil, errors.New("sqlite is not enabled")
	}
	source := strings.TrimSpace(cfg.Source)
	if source == "" {
		return SQLiteDB{}, nil, errors.New("sqlite.source is empty")
	}
	...
	gdb, err := gorm.Open(gormSQLite.Open(source), gormCfg)
	if err != nil {
		return SQLiteDB{}, nil, err
	}
	sqlDB, err := gdb.DB()
	if err != nil {
		return SQLiteDB{}, nil, err
	...
	helper := logger.WithField("module", "dbx/sqlite")
	helper.Info("sqlite connected")
	cleanup := func() {
		if err := sqlDB.Close(); err != nil {
			helper.Errorf("close sqlite error: %v", err)
		}
	}
	return SQLiteDB{Conn: gdb}, cleanup, nil
}
